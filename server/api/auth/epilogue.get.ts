...
defineRouteMeta({
  openAPI: {
    ...
    description: "Initializes Epilogue OAuth and redirects the client to Epilogue authorization page.",
    responses: {
...
export default defineEventHandler(async (event) => {
  try {
    ...
    epilogueAuthUrl.searchParams.append("cancel_url", `${frontendUrl}/login?error=cancelled`);
    ...
    const detailedMessage = error instanceof Error ? error.message : "An unknown error occurred during Epilogue auth initialization.";
    throw handleApiError(500, `Failed to initialize Epilogue authentication: ${detailedMessage}`, "Could not initiate Epilogue authentication. Please try again.");
  }
});