...
export default defineEventHandler(async (event: H3Event) => {
  ...
  try {
      ...
      where: {
        AND: [
          { timestamp: { lt: yesterday } },
          {
            NOT: {
              AND: [
                { userId: { in: await prisma.dailyProjectSummary.findMany({ select: { userId: true } }).then(s => s.map(x => x.userId)) } },
                { project: { in: await prisma.dailyProjectSummary.findMany({ select: { project: true } }).then(s => s.map(x => x.project)) } }
              ]
            }
          }
        ]
    ...
    heartbeatsToSummarize.forEach((heartbeat) => {
      ...
      userProjectHeartbeats[userId][dateKey][project].heartbeats.push(heartbeat);
    });
    ...
    for (const userId in userProjectHeartbeats) {
      for (const dateKey in userProjectHeartbeats[userId]) {
        for (const project in userProjectHeartbeats[userId][dateKey]) {
          const projectHeartbeats = userProjectHeartbeats[userId][dateKey][project].heartbeats;
          ...
          for (let i = 0; i < projectHeartbeats.length; i++) {
            ...
            const previous = new Date(projectHeartbeats[i - 1].timestamp).getTime();
