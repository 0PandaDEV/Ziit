...
export async function calculateStats(userId: string, timeRange: TimeRange) {
    ...
    for (const summary of summaries) {
      ...
      for (const heartbeat of summary.heartbeats) {
        if (heartbeat.project) {
          categoryCounters.projects[heartbeat.project] = (categoryCounters.projects[heartbeat.project] || 0) + 1;
        }
        if (heartbeat.language) {
          categoryCounters.languages[heartbeat.language] = (categoryCounters.languages[heartbeat.language] || 0) + 1;
        }
        if (heartbeat.editor) {
          categoryCounters.editors[heartbeat.editor] = (categoryCounters.editors[heartbeat.editor] || 0) + 1;
        }
        if (heartbeat.os) {
          categoryCounters.os[heartbeat.os] = (categoryCounters.os[heartbeat.os] || 0) + 1;
        }
      }
      ...
      if (totalHeartbeats > 0) {
        for (const [project, count] of Object.entries(categoryCounters.projects)) {
          const seconds = Math.round((count / totalHeartbeats) * totalSeconds);
          dailyData.projects[project] = (dailyData.projects[project] || 0) + seconds;
        }
        
        for (const [language, count] of Object.entries(categoryCounters.languages)) {
          const seconds = Math.round((count / totalHeartbeats) * totalSeconds);
          dailyData.languages[language] = (dailyData.languages[language] || 0) + seconds;
        }
        
        for (const [editor, count] of Object.entries(categoryCounters.editors)) {
          const seconds = Math.round((count / totalHeartbeats) * totalSeconds);
          dailyData.editors[editor] = (dailyData.editors[editor] || 0) + seconds;
        }
  ...
  return {
    summaries: dailyData,
    heartbeats: isSingleDayView ? heartbeats.map(h => ({
      ...h,
